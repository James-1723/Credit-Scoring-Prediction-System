...
### **From Local Machine**
"""
route_prefix = f"/Users/USER/Documents/DCG/程式語言/Grad_tem/testttt/Prediction/"
# 多個測資
file_path = f"{route_prefix}Source csv/test.csv"
# 單一測資
# file_path = f"{route_prefix}Source csv/test_single.csv"
# 完整測資
# file_path = f"{route_prefix}Source csv/客戶基本資料 10201-11305.csv"

# 多個測資
output_path = f"{route_prefix}Temporary file/tem.csv"
# 單一測資
# output_path = f"{route_prefix}Prediction/Temporary file/tem_single.csv"
# 完整測資
# output_path = f"{route_prefix}Prediction/Temporary file/temCSV.csv"
...
# 結果輸出
result_path = f"{route_prefix}Result csv/prediction.csv"
# 含shap預測
...
def taiwan_to_gregorian(taiwan_date):
    if taiwan_date == '無' or pd.isna(taiwan_date):
        return None
    if isinstance(taiwan_date, str):
        year, month, day = map(int, taiwan_date.split('/'))
    else:  # handle Timestamp case
        year, month, day = taiwan_date.year - 1911, taiwan_date.month, taiwan_date.day
    return datetime(year + 1911, month, day)

customer_data['進件日期'] = customer_data['進件日期'].astype(str).apply(taiwan_to_gregorian)
customer_data['出生日期'] = customer_data['出生日期'].astype(str).apply(taiwan_to_gregorian)

# 提取日期特徵
customer_data['進件（年）'] = customer_data['進件日期'].dt.year
customer_data['進件（月）'] = customer_data['進件日期'].dt.month
customer_data['進件（日）'] = customer_data['進件日期'].dt.day

customer_data['出生（年）'] = customer_data['出生日期'].dt.year
customer_data['出生（月）'] = customer_data['出生日期'].dt.month
customer_data['出生（日）'] = customer_data['出生日期'].dt.day
...
desired_columns = [
    ...
    '月薪', '年資', 'Current', 'M0', 'M1-1', 'M1-2', 'M2', 'M3', 'M4', 'M5', 'M6',
    'AMFC2 評分分數', 'AMFC3評分', '調整後原始分數'
]
...
drops = ['Current', 'M0', 'M1-1', 'M1-2', 'M2', 'M3', 'M4', 'M5', 'M6', 'AMFC3評分', '年資＿原資料', '月薪＿原資料']
for i in drops:
...
# single_sample = X.iloc[0:1]  # This takes the first row of X as an example
...
result_df = pd.DataFrame({
    ...
    '原始分數': dataset.loc[X.index, 'AMFC2 評分分數'].reset_index(drop=True),
    '預測分數': y_pred,
...
def analyze_single_record(model, X_df, dataset, y_pred):
    ...
    result_df = pd.DataFrame({
        ...
        '原始分數': dataset.loc[X_df.index, 'AMFC2 評分分數'].reset_index(drop=True),
        '預測分數': y_pred,
        '參考違約率': get_default_rate(y_pred)
    })
...
from scipy import stats  # 添加此行以導入 stats 模組
